...
export default function CartDrawer() {
  ...
  const handleCheckout = () => {
    // Close the drawer
    HSOverlay.close(document.querySelector('#cart-drawer')!)
  ...
  return (
    ...
    >
      ...
      <div id="cart-items-list" className="overflow-y-auto mx-3">
        <CartItemsList />
      </div>
      <div className="flex flex-col gap-4 bg-white w-full p-6">
        ...
        <div>
          <PrimaryButton text="Checkout" wFull onClick={handleCheckout} />
        </div>
        <div className="flex justify-center">
          <UnderlineButton text="View Cart" onClick={handleViewCart} />
        </div>
      </div>
    </div>
  )
